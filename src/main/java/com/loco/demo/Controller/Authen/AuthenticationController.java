...
public class AuthenticationController {
    ...
    public SchemaResponseStatusAuthenticationRegister registerUser(
        ...
        RegisterServiceReturnDTO response = authenticationService
                .registerUser(registerForm.username(), registerForm.password(), registerForm.email(),
                        registerForm.phoneNumber());
        User applicationUser = response.user();
        String message = response.message();
        if (applicationUser == null) {
            return new SchemaResponseStatusAuthenticationRegister(message, "000", StatusAuthenticationRegister.IS_EXIST,
                    null);
        }
        notificationService.makeNotification(applicationUser,
                "You have registered successfully with username " + registerForm.username(),
                new Date());
        return new SchemaResponseStatusAuthenticationRegister(message, "001", StatusAuthenticationRegister.ACCEPTED,
                new SecureUser(applicationUser));
    }
    ...
    public SchemaResponseStatusAuthenticationLogin loginUser(
        ...
        User applicationUser = authenticationService
                .setUpLoginUser(registrationDTO.getUsername(), registrationDTO.getPassword()) != null
                        ? authenticationService.setUpLoginUser(registrationDTO.getUsername(),
    ...
    public GetMeDataDTO getMeDataDTO() {
        try {
            ...
            if (getUserFromDB == null) {
                return new GetMeDataDTO(StatusResponseAPI.NOT_EXIST, "Username fetched from token is not exist", "000",
                        null);
            } else {
    ...
    public AdvanceLoginResponseDTO googleLogin(@RequestBody GoogleLoginDTO googleLoginDTO) {
        LoginResponseDTO loginResponseDTO = authenticationService
                .handleLoginAndRegisterWithGoogleAccount(googleLoginDTO);
