...
public class ExceptionHandleManagers {
    ...
    ExceptionResponseHandler handleInvalidBearerTokenException(InvalidBearerTokenException ex) {
        return new ExceptionResponseHandler(StatusResponseAPI.UNAUTHORIZED,"000","The access token provided is expired, revoked, malformed, or invalid for other reasons.", ex.getMessage());
    }
    ...
    ExceptionResponseHandler handleNoHandlerFoundException(NoHandlerFoundException ex) {
        return new ExceptionResponseHandler(StatusResponseAPI.NOT_FOUND,"000", "This API endpoint is not found.", ex.getMessage());
    }

    @ExceptionHandler(Exception.class)
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    ExceptionResponseHandler handleInternalServerError(Exception ex) {
        return new ExceptionResponseHandler(StatusResponseAPI.INTERNAL_SERVER_ERROR,"000","A server internal error occurs.", ex.getMessage());
    }
